...
namespace UnityToolkit
{
    ...
    public class TypeEventSystem
    {
        ...
        {
            _repository.Get<Event<T>>()?.Trigger(new T());
        }

        public void Send<T>(T args)
        {
            _repository.Get<Event<T>>()?.Trigger(args);
        }

        public ICommand Register<T>(Action<T> onEvent)
        {
            return _repository.GetOrAdd<Event<T>>().Regiser(onEvent);
        }

        public void Unregister<T>(Action<T> onEvent)
        {
            _repository.Get<Event<T>>()?.Unregister(onEvent);
        }
    }
