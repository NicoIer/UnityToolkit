...
namespace Nico.Editor.DialogSystem
{
    ...
    {
        public static DialogGraphData GetGraphData(this DialogGraphView dialogGraphView, string fileName)
        {
            DialogGraphData graphData = ScriptableObject.CreateInstance<DialogGraphData>();
            graphData.fileName = fileName;
            graphData.globalNodeData = dialogGraphView.GetGlobalNodeData();
            graphData.groupNodeData = dialogGraphView.GetGroupNodeData();
            graphData.groupDatas = dialogGraphView.GetGroupData();
            return graphData;
        }

        public static SerializableDictionary<string, DialogGraphData.NodeData> GetGlobalNodeData(
            this DialogGraphView dialogGraphView)
        {
            var data = new SerializableDictionary<string, DialogGraphData.NodeData>();
            foreach ((string nodeName, var errorData) in dialogGraphView.UgNodes)
            {
                ...
                var nodeData = new DialogGraphData.NodeData
                {
                    name = nodeName,
        ...
        public static SerializableDictionary<string, SerializableDictionary<string, DialogGraphData.NodeData>>
            GetGroupNodeData(this DialogGraphView dialogGraphView)
        {
            var data = new SerializableDictionary<string, SerializableDictionary<string, DialogGraphData.NodeData>>();
            foreach (var (group, nodeErrorDatas) in dialogGraphView.GNodes)
            {
                var groupedNodes = new SerializableDictionary<string, DialogGraphData.NodeData>();
        ...
        public static List<DialogGraphData.GroupData> GetGroupData(
            this DialogGraphView dialogGraphView)
        {
            var data = new List<DialogGraphData.GroupData>();
            foreach (var (groupName, groupErrorData) in dialogGraphView.GroupsData)
            {
                ...
                var groupData = new DialogGraphData.GroupData
                {
                    name = groupName,
