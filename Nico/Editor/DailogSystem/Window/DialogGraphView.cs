...
namespace Nico.Editor.DialogSystem
{
    ...
    {
        private readonly DialogSystemWindow editorWindow;
        ...
        public DialogGraphView(DialogSystemWindow editorWindow, StyleSheet sheet)
        {
            ...
            ContextualMenuManipulator manipulator = new ContextualMenuManipulator(
                menuEvent =>
                {
                    menuEvent.menu.AppendAction("Add Group",
                        actionEvent => { AddElement(CreateGroup("Group", actionEvent.eventInfo.mousePosition)); });
                });
            return manipulator;
        }
        ...
        public void LoadGraphData(DialogGraphData data)
        {
            //首先复原数据
            //复原组
            ...
            //复原节点
            foreach (var (_, nodeData) in data.globalNodeData)
            {
                if (nodeData.inGroup)
                {
            ...
            foreach (var (groupName, groupData) in data.groupNodeData)
            {
