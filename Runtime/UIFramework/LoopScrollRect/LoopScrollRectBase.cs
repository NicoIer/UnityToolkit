...
namespace UnityToolkit
{
    ...
    {
        #region Delagate Defines

        public delegate void ItemRendererDelegate(Transform transform, int idx);

        public delegate GameObject ItemProviderDelegate(int idx);

        public delegate void ReturnDelegate(Transform transform);

        public delegate Vector2 GetItemSizeDelegate(int itemCount);
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                RectTransform oldItem =
                    m_Content.GetChild(m_Content.childCount - deletedItemTypeEnd - 1) as RectTransform;
                size = Mathf.Max(GetSize(oldItem), size);
        ...
        internal static Vector2 InternalCalculateOffset(ref Bounds viewBounds, ref Bounds contentBounds,
            ...
            float deltaDimension, int itemTypeStart, int itemTypeEnd,
            int totalCount) //==========LoopScrollRect==========
        {
