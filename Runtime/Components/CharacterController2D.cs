...
namespace UnityToolkit
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
...
#if UNITY_EDITOR
        public bool enableCc2DRaysDebug = false;
#endif
        ...
        /// </summary>
        [SerializeField] private float _slopeLimitTangent = Mathf.Tan(75f * Mathf.Deg2Rad);


        [NonSerialized] private Transform _transform;
        [NonSerialized] private BoxCollider2D _boxCollider;
        // [NonSerialized] public Rigidbody2D rigidBody2D;

        [NonSerialized] private readonly CharacterCollisionState2D _collisionState = new CharacterCollisionState2D();

        [NonSerialized] public Vector3 velocity;

        public bool isGrounded => _collisionState.below;

        const float _kSkinWidthFloatFudgeFactor = 0.001f;
        ...
        public void Move(Vector3 deltaMovement)
        {
            ...
            deltaMovement.z = 0; // TODO 在FixedUpdate中处理
        ...
        public void RecalculateDistanceBetweenRays()
        {
            ...
            var colliderUsableHeight = _boxCollider.size.y * Mathf.Abs(localScale.y) - (2f * _skinWidth);
            _verticalDistanceBetweenRays = colliderUsableHeight / (totalHorizontalRays - 1);

            // vertical
            var colliderUsableWidth = _boxCollider.size.x * Mathf.Abs(localScale.x) - (2f * _skinWidth);
        ...
        void MoveHorizontally(ref Vector3 deltaMovement)
        {
            ...
            for (var i = 0; i < totalHorizontalRays; i++)
            {
                ...
                if (i == 0 && _collisionState.wasGroundedLastFrame)
                    ...
                    _raycastHit = Physics2D.Raycast(ray, rayDirection, rayDistance,
                        platformMask & ~oneWayPlatformMask);
        ...
        private void HandleVerticalSlope(ref Vector3 deltaMovement)
        {
            ...
            if (_raycastHit)
            {
                ...
                var isMovingDownSlope =
                    Math.Abs(Mathf.Sign(_raycastHit.normal.x) - Mathf.Sign(deltaMovement.x)) < 0.01f;
